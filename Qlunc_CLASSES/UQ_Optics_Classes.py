...
#   Estimating the error in theta and phi, for each measured height, by 'equivalent triangles' method
    ...
    dydtheta = rho*np.sin(np.deg2rad(Lidar.optics.scanner.phi))*np.cos(np.deg2rad(Lidar.optics.scanner.theta))
    dydphi   = rho*np.cos(np.deg2rad(Lidar.optics.scanner.phi))*np.sin(np.deg2rad(Lidar.optics.scanner.theta))
    
    dzdrho   = np.cos(np.deg2rad(Lidar.optics.scanner.theta))
    dzdtheta = -(rho*np.sin(np.deg2rad(Lidar.optics.scanner.theta)))
    dzdphi   = 0
