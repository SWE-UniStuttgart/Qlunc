    ...
    * Lidar
    ...
    if flag_plot_measuring_points_pattern:
        if Lidar.optics.scanner.pattern in ['None']:
            ...
            color4=iter(cm.Set2(np.linspace(0,1,3)))
            # plt.tight_layout()
            ...
            SensCoeff1=[Data['Sens coeff Vlos']['V1_theta'][0],Data['Sens coeff Vlos']['V1_psi'][0],Data['Sens coeff Vlos']['V1_rho'][0]]

            
            Cont_Theta1         = (np.array(Data['Sens coeff Vlos']['V1_theta'][-1]*np.array(Data['STDVs'][0][0])))**2
            Cont_Psi1           = (np.array(Data['Sens coeff Vlos']['V1_psi'][-1]*np.array(Data['STDVs'][1][0])))**2
            Cont_Rho1           = (np.array(Data['Sens coeff Vlos']['V1_rho'][-1]*np.array(Data['STDVs'][2][0])))**2     
            Cont_Corr1          = 2*Lidar.optics.scanner.correlations[9]*np.array(Data['Sens coeff Vlos']['V1_theta'][0])*np.array(Data['Sens coeff Vlos']['V1_psi'][0])*np.array(Data['STDVs'][0][0])*np.array(Data['STDVs'][1][0])
            # pdb.set_trace()
           ...
           # 6.  Plot Uncertainty in VLOS2 with wind direction 
            ...
            # Plot with sensitivity coefficients: Data['Uncertainty contributors Vlos1']=[contribution theta, contribution varphi, contribution rho] for alpha 0, 0.1 and 0.2. For the plotting we use alpha=0.2            
            Cont_Theta2         = (np.array(Data['Sens coeff Vlos']['V2_theta'][-1]*np.array(Data['STDVs'][0][1])))**2
            Cont_Psi2           = (np.array(Data['Sens coeff Vlos']['V2_psi'][-1]*np.array(Data['STDVs'][1][1])))**2
            Cont_Rho2           = (np.array(Data['Sens coeff Vlos']['V2_rho'][-1]*np.array(Data['STDVs'][2][1])))**2     
            Cont_Corr2          = 2*Lidar.optics.scanner.correlations[9]*np.array(Data['Sens coeff Vlos']['V1_theta'][0])*np.array(Data['Sens coeff Vlos']['V1_psi'][0])*np.array(Data['STDVs'][1][1])*np.array(Data['STDVs'][0][1])
            # pdb.set_trace()
