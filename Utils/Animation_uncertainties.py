...
def PlotAnimUnc(Wdir_U,Vh_u,Coord):
    ...
    # Wind direction uncertainty animation 
        ...
        for ini in np.arange(n,-1,-1):
            plt.axhline(Wdir_U[ini],color = 'g', linestyle = '-',alpha=0.3)
        summ1.append([np.sum(Wdir_U[indd]) for indd in range(ii1+1)])
        ii1+=1
        props = dict(boxstyle='round', facecolor='green', alpha=0.4)
        textstr1 = 'mean[°] = '+str(np.round(np.nanmean(summ1[n]),2))+'; stdv[°] = '+str(np.round(np.nanstd(summ1[n]),2))
    ...
    # Horizontal velocityh uncertainty animation
        ...
        for ini in np.arange(n,-1,-1):
            # pdb.set_trace()
            plt.axhline(Vh_u[ini],color = 'b', linestyle = '-',alpha=0.4)
        summ2.append([np.sum(Vh_u[indd]) for indd in range(ii2+1)])
        ii2+=1
        props = dict(boxstyle='round', facecolor='blue', alpha=0.3)
        # pdb.set_trace()
        textstr2 ='mean[m/s] = '+str(np.round(np.nanmean(summ2[n]),2))+'; stdv[m/s] = '+str(np.round(np.nanstd(summ2[n]),2))
